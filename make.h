...
void error (const struct floc *flocp, const char *fmt, ...)
            __attribute__ ((__format__ (__printf__, 2, 3)));
int fatal (const struct floc *flocp, const char *fmt, ...);
#else
const char *concat ();
void message ();
void error ();
...
extern jmp_buf jump_mark;
extern int nFinalReturnValue;

typedef void (*fpSendNewLine) (char* pLine, void* pSlnMaker);

extern fpSendNewLine pSendNewLine;
extern void* pSolutionMaker;

#define EXIT_HAPPENED -10000