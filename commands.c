...
int execute_file_commands (struct file *file)
{
  const char *p;
...
int fatal_error_signal (int sig)
{
  ...
  if (sig == SIGTERM || sig == SIGINT
	...
	if (EXIT_HAPPENED == reap_children (1, 0))
    {
    ...
    while (job_slots_used > 0)
    {
        if (EXIT_HAPPENED == reap_children (1, 1))
        {
            return EXIT_HAPPENED;
        }
    }
...
static int delete_target (struct file *file, const char *on_behalf_of)
{
  struct stat st;
  int e;
  int nRetValue = 0;
...
#ifndef NO_ARCHIVES
  nRetValue = ar_name (file->name);
  if (nRetValue == EXIT_HAPPENED)
  {
      return nRetValue;
  }
  else if (nRetValue)
    {
      ...
      tRetValue = ar_member_date (file->name);
      if (EXIT_HAPPENED == tRetValue)
      {
          return EXIT_HAPPENED;
      }
      if (tRetValue != file_date)
	{
	  if (on_behalf_of)
...
int delete_child_targets (struct child *child)
{
  struct dep *d;
  ...
  if (EXIT_HAPPENED == delete_target (child->file, NULL))
  {
      return EXIT_HAPPENED;
  }
  ...
  {
      if (EXIT_HAPPENED == delete_target (d->file, child->file->name))
      {
          return EXIT_HAPPENED;
      }
  }
