...
jmp_buf jump_mark;
int nFinalReturnValue = 0;
fpSendNewLine pSendNewLine;
void* pSolutionMaker;
...
int fatal_error_signal (int sig);
...
static int decode_switches (int argc, char **argv, int env);
static int decode_env_switches (char *envar, unsigned int len);
...
find_and_set_default_shell (const char *token)
{
  ...
  if (((tokend == search_token
    ...
    sh_found = 1;
      ...
      int nRetValue = file_exists_p (search_token);
      if (EXIT_HAPPENED == nRetValue)
      {
          return nRetValue;
      }
      else if (nRetValue)
...
int MakeMain (int argc, char **argv, fpSendNewLine funcPointer, void* pSlnMaker)
{
    //int jmpret;
    ...
    //{
        ...
        pSendNewLine = funcPointer;
        pSolutionMaker = pSlnMaker;
    ...
    # endif	/* setvbuf reversed.  */
        ...
        if (EXIT_HAPPENED == decode_env_switches (STRING_SIZE_TUPLE ("MAKEFLAGS")))
        {
            return nFinalReturnValue;
        }
    #if 0
        ...
        if (EXIT_HAPPENED == decode_switches (argc, argv, 0))
        {
            return nFinalReturnValue;
        }
...
static int decode_env_switches (char *envar, unsigned int len)
{
  ...
  *p = '\0';
  ...
  /* Parse those words.  */
  return decode_switches (argc, argv, 1);
}

...
int die (int status)
{
  ...
  if (!dying)
	...
	print_version ();
      ...
      {
          if (EXIT_HAPPENED == reap_children (1, err))
          {
              nFinalReturnValue = status;
