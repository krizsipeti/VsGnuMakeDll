...
ar_name (const char *name)
{
  ...
  if (p[1] == '(' && end[-1] == ')')
  {
      if (EXIT_HAPPENED == fatal (NILF, _("attempt to use unsupported feature: `%s'"), name))
      {
...
ar_member_date (const char *name)
{
     ...
     base exist or can be made.  */
  {
    ...
    nRetValue = file_exists_p (arname);
    if (EXIT_HAPPENED == nRetValue)
    {
        return nRetValue;
    }
    if (arfile == 0 && nRetValue)
      arfile = enter_file (strcache_add (arname));

    if (arfile != 0)
    {
        if (EXIT_HAPPENED == f_mtime (arfile, 0))
        {
            return EXIT_HAPPENED;
        }
    }
