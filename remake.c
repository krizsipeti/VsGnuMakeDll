...
static int remake_file (struct file *file);
  ...
  while (goals != 0)
    {
      ...
      if (EXIT_HAPPENED == start_waiting_jobs ())
      {
  ...
  for (; f != 0; f = f->prev)
    {
      ...
      nRetValue = update_file_1 (f, depth);
    ...
    for (; f != 0 ; f = f->prev)
      {
        ...
        {
            nRetValue = update_file (d->file, depth + 1);
            if (EXIT_HAPPENED == nRetValue)
            {
                return nRetValue;
            }
...
update_file_1 (struct file *file, unsigned int depth)
{
      ...
      while (d)
        {
          ...
          nRetValue = check_dep (d->file, depth, this_mtime, &maybe_make);
          if (EXIT_HAPPENED == nRetValue)
          {
              return nRetValue;
          }
          dep_status |= nRetValue;
	...
	if (d->file->intermediate)
	  {
	    ...
	    d->file->parent = file;
            ...
            nRetValue = update_file (d->file, depth);
            if (EXIT_HAPPENED == nRetValue)
            {
                return EXIT_HAPPENED;
            }
  ...
  if (EXIT_HAPPENED == remake_file (file))
  {
      return EXIT_HAPPENED;
  }
