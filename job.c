...
int reap_children (int block, int err)
{
#ifndef WINDOWS32
  WAIT_T status;
      ...
      if (child_failed && !c->noerror && !ignore_errors_flag)
        {
          ...
          {
              if (EXIT_HAPPENED == delete_child_targets (c))
              {
...
int new_job (struct file *file)
{
  ...
  if (EXIT_HAPPENED == reap_children (0, 0))
  {
      return EXIT_HAPPENED;
  }
